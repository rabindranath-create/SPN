...
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_40_1.csv"))
for (i in 1:10){
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_40_2.csv"))
for (i in 1:10){
  obs_info_all_use <- obs_info_all[(10*(i-1)+1):(10*i)]
  result <- parLapply(cl,obs_info_all_use,WCSPP_Node_DT)
  for (j in 1:10){
...
result_WCSPP_DT <- matrix(NA,ncol=7,nrow=100)
write.csv(result_WCSPP_DT, file = file.path(output_dir, "result_WCSPP_DT_40_3.csv"))
for (i in 1:10){
  obs_info_all_use <- obs_info_all[(10*(i-1)+1):(10*i)]
  result <- parLapply(cl,obs_info_all_use,WCSPP_Node_DT)
  for (j in 1:10){
